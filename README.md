...
A user interacts with a page such as Login, Register, Resume Vault, or AI Optimizer.
The page captures input using controlled React state.
A submit or action handler triggers an Axios request to the configured backend base URL.
The frontend processes success or failure responses.
The UI updates immediately with navigation, state refreshes, success toasts, or inline error feedback.
...
Build endpoint URLs through apiUrl(...)
Trigger requests from service or page handlers
Track loading or submitting state in React state
Render success, error, or fallback UI based on the response
...
Form fields update local React state on every change
User submits the form
Frontend validation runs first where applicable
Axios sends the request to the configured backend endpoint
On success, token or response data is processed
UI updates by navigating to the next route or showing a success message
On failure, backend or fallback error text is rendered in the UI
2. Protected Route Flow
User attempts to access a protected screen
ProtectedRoute checks whether a token exists in auth context
If the token is present, the requested screen renders
If not, the user is redirected to the login page
3. Resume Vault Flow
User opens the resume vault
Frontend fetches resume data using the auth token in request headers
Loading state renders while the request is in flight
On success, resume cards populate the UI
On upload, delete, or download actions, the related Axios request runs
Success or failure feedback is shown immediately through UI messaging
4. AI Workflow Flow
User provides resume or job-related input
Frontend sends the request to the relevant optimization endpoint
The response is transformed into editable frontend state
User can review, refine, and continue the workflow based on returned output
